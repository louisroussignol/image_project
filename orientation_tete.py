...
def control_mouse_with_nose_position():
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("Erreur : impossible d'ouvrir la caméra.")
        return

    # mofication de la résolution
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)

    previous_noze_x=None
    previous_noze_y=None
    init_time = 0
    previous_time = 0
    ...
    with mp_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # Récupérer les coordonnées du nez
                nose_tip = face_landmarks.landmark[1]  # Pointe du nez
                nose_x, nose_y, nose_z = nose_tip.x, nose_tip.y, nose_tip.z
                frame_height, frame_width, _ = frame.shape
                
                # Convertir les coordonnées normalisées en pixels
                nose_x_px = nose_x * frame_width
                nose_y_px = nose_y * frame_height
                if previous_noze_x:
                    # Calculer les déplacements relatifs
                    delta_x = (nose_x_px - previous_noze_x) /(1+nose_z)
                    delta_y = (nose_y_px - previous_noze_y) /(1+nose_z)
                    #print(nose_z)
                    # Multiplier par un facteur de sensibilité pour ajuster les mouvements
                    sensitivity = 10 
                ...
                previous_noze_x,previous_noze_y = (nose_x_px, nose_y_px)
            cv2.imshow("Controle de la souris avec la position du nez", frame)
...
if avg_left_eye_distance!=None and avg_right_eye_distance!=None:
    control_mouse_with_nose_position()
clear_terminal()
